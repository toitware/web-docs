...
exist at any given point in time. This ensure that two applications can't configure or use a
specific GPIO pin at the same time. That also means that a GPIO pin should be `close`d after usage:
