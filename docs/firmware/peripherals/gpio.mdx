...
specific GPIO pin at the same time. To pin can be released againby calling `close` after usage:
...
```
  pin := gpio.Pin 21 --output // Initialized for output mode.
```
...
```
  pin := gpio.Pin 21 --input // Initialized for input mode.
```
or later (independent of previous mode):
```
