...
The ESP32 supports Ethernet in a couple of ways:

  * Using the ESP32 for the MAC layer but an external chip for the PHY layer, using RMII.
...
Only boards with an external reference clock connected to GPIO 0 is supported.
...
When an external MAC chip is used, it's communicated with using an dedicated SPI bus.
...
The ESP32 comes with a an allocated Ethernet MAC address that by default will be used by both internal and external MAC configurations.
