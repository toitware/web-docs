...
The Firmware have encoded how to _use_ these pins, but which pin to use is fully optional and thus needs to be configured:
...
Both the MCU and the Cellular modem have such pins, and must be connected as TX to RX and RX to TX, such that data _transmitted_ from one will be _received_ by the other.

<img src={CellularUart} alt="Cellular UART TX/RX" />

<Note type="info">

Some Cellular modules show TX and RX as they should be connected directly by the MCU. If connection doesn't work, **try swapping RX and TX**.
...
and unlike TX and RX, they are _not_ cross connected. They work by pulling down RTS when Cellular Modem can write, and likewise pulling down CTS when the MCU can write.
...
For the command, simple omit RTS and CTS:
