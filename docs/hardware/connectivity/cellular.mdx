...
Both the MCU and the Cellular modem have such pins, and must be connected as TX to RX and RX to TX, such that data *transmitted* from one will be *received* by the other.
...
and unlike TX and RX, they are *not* cross connected. They work by pulling down RTS when Cellular Modem can write, and likewise pulling down CTS when the MCU can write.
