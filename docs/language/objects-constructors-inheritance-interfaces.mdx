...
"primitive types" in other languges. For example, an [integer](math.md)
...
<div class="toit">``` class Point: x/int := 42 y/int := 103 ```</div>
...
<div class="toit">``` main: p := Point // Creates a new Point object print p.x // >> 42 print p.y // >> 103 ```</div>
...
the methods they declare.  Since a method signature declared in an
...
have methods without implementations.  The class and the
methods must be marked explicitly as abstract.  A class cannot
...
x = length _ (math.cos angle)
y = length _ (math.sin angle)
...
class Vector:
x/float // These must be initialized in the intializer...
