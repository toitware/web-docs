...
Part of the [language reference](/language/reference), for readers who can already
follow a small Toit program. New to Toit? Start with the
[beginner tutorial](/language/beginner) or [Toit for programmers](/language/toitversus).
...
by calling its `stringify` method. Interpolation follows Toit identifier rules,
including hyphens and underscores, and can include member access and indexing
...
To separate interpolation from literal text that could be read as part of the
expression, or to interpolate a more complicated expression, put parentheses
around the expression. For example, `"$(name)-sensor"` keeps the hyphen and
`sensor` outside the interpolated name:
