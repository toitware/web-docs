...
are reserved for
[character literals](/language/syntax/#string_and_character_literals).)

If a string literal contains double quotes (`"`), or backslashes (`\`),
then these must be escaped with backslashes:

<div class="toit">
  ``` main: print "String with \"quotes\"." // >> String with "quotes". print "Zig zag string /\\/\\." // >> Zig zag
  string /\/\. ```
</div>
...
| `\'`                                  | '                                                                     |                   | 39                        |
| `\\`                                  | \                                                                     |                   | 92                        |
| `\uXXXX` (where X is a hex char)      | [Unicode rune](<https://en.wikipedia.org/wiki/Runic_(Unicode_block)>) |                   |                           |
| `\u{X...}` (where X... are hex chars) | [Unicode rune](<https://en.wikipedia.org/wiki/Runic_(Unicode_block)>) |                   |                           |
| `\xXX` (where X is a hex char)        | [Unicode rune](<https://en.wikipedia.org/wiki/Runic_(Unicode_block)>) |                   |                           |
| `\x{X...}` (where X... are hex chars) | [Unicode rune](<https://en.wikipedia.org/wiki/Runic_(Unicode_block)>) |                   |                           |
...
See [below](#multi-line_strings) for more advanced
string literals.
...
Since null is [falsy](/language/booleans/) we can index
...
[do
--runes](https://libs.toit.io/core/class-string#do(block%2Cbool))
method:
...
byte array with the
[to_byte_array](https://libs.toit.io/core/class-string#to_byte_array())
or [write_to_byte_array](https://libs.toit.io/core/class-string#write_to_byte_array(any))
methods.
...
In multiline strings there are some extra escapes that can
be used.

|                                              |                                                                                                 |
| -------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `\` followed by a newline (ASCII 10)         | Removes a newline in a multiline string.                                                        |
| `\` followed by a carriage return (ASCII 13) | Removes a newline in a multiline string. If followed by a newline, that one is removed as well. |
| `\s`                                         | Space (see below for a detailed explanation).                                                   |

Toit removes the indentation of a multiline string.
...
<div class="toit">
  ``` main: str1 := """ foo""" str2 := """ \sfoo""" print "[" + str1 + "]" // >> [foo] print "[" + str2 + "]" // >> [
  foo] ```
</div>
...
<div class="toit">
  ``` main: i := 42 print "Level $i, the band." // >> Level 42, the band. j := -42 print "Level $j.abs, the band." // >>
  Level 42, the band. list := [0, 42, 103] print "Level $list[1], the band." // >> Level 42, the band. ```
</div>
...
<div class="toit">
  ``` main: print "Level $(6 * 7), the band." // >> Level 42, the band. j := -42 print "Level $(-j), the band." // >>
  Level 42, the band. i := 42 print "Almost $(i)mm." // >> Almost 42mm. ```
</div>
...
[stringify](https://libs.toit.io/core/numbers/class-int#stringify(any))

The full set of formatting options includes left-padding,
and center-padding.  See the documentation on the
static string method, [format](https://libs.toit.io/core/class-string#format(string%2Cany))
```
