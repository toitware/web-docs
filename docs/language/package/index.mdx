...
The Toit package manager (henceforth TPM) is integrated with the [`toit` CLI](../../getstarted/details/installation).
...
It also contains information such as the package's name, a description, and its dependencies. (See the [specification section](pkgconcepts/#package) for more details).
...
Registries create descriptions of the known packages and store them in a Git repository. The [Toit registry](https://github.com/toitware/registry/tree/main/packages/github.com) is just a public Git repository with description files.
