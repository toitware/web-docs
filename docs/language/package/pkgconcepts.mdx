...
When a program uses a package, that dependency is registered in the program's lock file `package.lock`. This is where the mapping from `prefix` to package is stored. Programs have concrete dependencies. All their dependent packages are resolved to a specific version, not a range of versions. These versions are written in the lock file.

When a developer calls `toit pkg install` or `toit.pkg install` they always get the same packages. A team of developers working on the same project will thus work with the exact same versions.
...
contain programs they will normally also have `package.lock` files, which
...
you could clone it from Github, then edit your `package.yaml` file from:
...
After editing the `package.yaml` file, run `toit pkg install` or `toit.pkg install` to
...
Similar to packages, registries also exist in two variants: Git and local registries. Git registries are added to TPM with a Git-URL, and can be synchronized with `toit pkg sync` or `toit.pkg sync` (see the [quick start](../pkgguide) section for more).
