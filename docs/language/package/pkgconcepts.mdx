...
When a developer calls `toit pkg install`, `toit.pkg install` or `jag pkg install` they always get the same packages. A team of developers working on the same project will thus work with the exact same versions.
...
Similar to packages, registries also exist in two variants: Git and local registries. Git registries are added to TPM with a Git-URL, and can be synchronized with `toit pkg sync` or `jag pkg sync` (see the [quick start](../pkgguide) section for more).
