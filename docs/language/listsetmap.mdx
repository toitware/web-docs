...
<div class="toit">``` list := List set := Set map := Map ```</div>
...
<div class="toit">``` score := scores[team] print "The score for $team is $score" ```</div>

The index operator syntax is understood by the string interpolation syntax, so you can simply write:

<div class="toit">``` print "The score for $team is $scores[team]" ```</div>

Updating or adding to a map is done with the `[]=` operator:

<!-- HIDDEN CODE title_map := {:} -->

<div class="toit">``` graduate name/string: title_map[name] = "PhD" ```</div>
...
<div class="toit">
  ``` register_goal team/string new_scorer/string -> none: // Get the set of goal-scorers for this team, // initializing
  with the empty set if necessary. goal_scorers/Set := scorers.get team --init=: {}
  goal_scorers.add new_scorer ```
</div>
...
<div class="toit">
  ``` report_odd collection/List: found := false collection.do: if it % 2 == 1: found = true if found: print "The
  collection contains at least one odd number" ```
</div>
...
<div class="toit">
  ``` if (collection.any: it % 2 == 1): print "The collection contains at least one odd number" ```
</div>
...
<div class="toit">``` if (collection.every: it % 2 == 1): print "The collection contains only odd numbers" ```</div>
...
<div class="toit">``` sum := collection.reduce: | a b | a + b ```</div>
...
<div class="toit">
  ``` sum_of_roots collection/Collection -> float: return collection.reduce --initial=0.0: | sum_so_far integer |
...
<div class="toit">
  ``` // Creates a comma-separated list of quoted strings. csv list/List -> string: Q ::= "\"" // A double quote. if
  (list.any: it.contains Q): throw "Can't handle strings containing quotes" quoted_list := list.map: "$Q$it$Q" return
  quoted_list.join "," ```
</div>
...
<div class="toit">
  ``` // Creates a comma-separated list of quoted strings, // escaping quotes. csv list/List -> string: Q ::= "\"" // A
  double quote. escaped_list := list.map: str := "$it" if str.contains Q: // In CSV quotes are escaped as two quotes.
  str.replace --all Q "$Q$Q" else: str quoted_list := escaped_list.map: "$Q$it$Q" return quoted_list.join "," ```
</div>
...
<div class="toit">
  ``` // Creates a comma-separated list of quoted strings, // escaping quotes. csv list/List -> string: return
  (list.map: "\"$("$it".replace --all "\"" "\"\"")\"").join "," ```
</div>
