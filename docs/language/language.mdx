...
Toit is an [object-oriented](/language/objects-constructors-inheritance-interfaces/)
...
It is called [_string interpolation_](/language/strings/#string_interpolation).
...
This is a
[constructor](/language/objects-constructors-inheritance-interfaces/#constructors)
...
[collections](/language/listsetmap/), the [special variable
`it`](/language/blocks/#block_arguments) contains the individual elements from the list in
...
Here we use a [hash map](/language/listsetmap/) to store the appropriate title for each name.  The empty map is given by `{:}` and we use `[]` to access the values for each key.  The empty set is `{}` and we already met the empty list, `[]`.  The lookup syntax `[]` also works on lists, so instead of the 'do' method we could have used:
