...
two-dimentional array of values.
...
We can specify a pixel stride for source and destination.  This
lets us read or write only every 2nd (or nth) byte.  For example
we might want to double the size of a byte array, writing alternately
...
lines are not adjacent.  We might be writing only the first 4 bytes of each
line, but each line has 10 bytes in it.  In this case we can use
`--destination_line_stride` and `--source_line_stride` to specify the
distance in bytes between adjacent lines.  We can also use this to
manipulate only every second line, by specifying a line stride that is
twice as large as the line length.

By making use of both slices and line strides, we can copy an arbitrary
...
By using `destination_pixel_stride` we can write every second byte
