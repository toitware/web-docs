...
This guide assumes you can read Toit functions, loops, and lambdas. It explains
cooperative scheduling and then covers synchronization primitives. For an
orientation from another language, read [Toit for programmers](/language/toitversus).

## Introduction

Concurrent activities need independent control flow. JavaScript's async/await,
for example, lets code wait for promises while other work proceeds. Toit uses
tasks with independent call stacks: ordinary functions can wait without an
`async` declaration or an `await` expression at each call site.

Tasks within a Toit program share objects and run cooperatively. This simplifies
updates that do not yield, but shared state can still change while a task waits.
...
JavaScript can express similar loops using async functions and awaited timers.
In Toit, the waiting operation suspends an ordinary call stack, so helpers and
their callers do not need async/await syntax.
...
The classes in `import monitor` are themselves written with the `monitor`
keyword. You can use it to write your own synchronization objects. A monitor is
declared like a class, but with `monitor` instead of `class`:
...
monitor BoundedQueue:
  ...
  constructor .capacity_:

  /// Adds $item to the queue, blocking while the queue is full.
  put item -> none:
    await: items_.size < capacity_
    items_.add item

  /// Removes and returns the oldest item, blocking while the queue is empty.
  take -> any:
    await: not items_.is-empty
    return items_.remove-first
...
Every public method of a monitor holds a lock on the monitor instance while it
runs. Only one task at a time can be inside a public method of a given
instance; other tasks calling public methods on the same instance wait until
the lock is released.

Since tasks only switch at yield points, you rarely need a lock for code that
doesn't yield. Monitors become important when a method needs to wait for
something, or when it calls code that might yield, such as `print` or I/O.

Private methods (names ending with `_`) do not take the lock. They are meant to
be called from public methods that already hold it.

<Note type="warning">

The lock is not reentrant. A public method that calls another public method on
the same monitor deadlocks. Move the shared code into a private method instead:
...
monitor Counter:
  ...
  add-two -> none:
    // Calling `increment` here would deadlock.
    increment_
    increment_
...
Inside a monitor method, `await: condition` blocks the task until the
condition block evaluates to true. While waiting, the lock is released, so
other tasks can call methods on the monitor and change its state. When the
condition becomes true, the task re-acquires the lock and continues.

In the `BoundedQueue` example above, `take` waits for an item, and `put`
waits for free space. Each call to `put` or `take` changes the state, which
lets the waiting task in the other method continue.

The condition is only re-evaluated after a public method on the same monitor
instance has run. It should therefore only depend on the state of the monitor
itself. The condition must not change any state.

If the task has a deadline, for example because it runs inside `with-timeout`,
`await` throws `DEADLINE_EXCEEDED` when the deadline is reached. Use
`try-await --deadline=...` to get a boolean result instead of an exception.

### Restrictions

A monitor cannot have a super class, and monitors should not be extended.
