...
segments that tell Toit how to find the library. Optionally it can then be
...
(See #<link> folder shortcut below).
...
See <customizations> below for ways of changing that.
...
can dot into sub-folders the same way as for local imports.

For example, a common sub-folder import is for the JSON library:
...
top-level `parse` function of that library (https://libs.toit.io/encoding/json/library-summary#parse(1%2C0%2C0%2C)).

Note, that global imports also apply the folder shortcut, as
described <#link-below>.
...
Inside `main.toit` we can import `my_lib.toit` by writing `import .sub.sub`.
...
(https://github.com/toitware/toit-morse). When installing this package
with `toit pkg install github.com/toitware/toit-morse` the package manager
