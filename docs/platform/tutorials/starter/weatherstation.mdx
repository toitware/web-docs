...
follow this [guide] (../../../getstarted/vscode) before proceeding with this tutorial. Also, you need the Toit CLI which can be downloaded and
installed as described [here](../../../getstarted/installation).

Then, open a terminal window in VSCode and execute the following two commands in order to install the BME280 driver package locally:
...
In the Toit code above, we import the driver package with `import bme280`.

We also specify the [GPIO pins](../../../../hardware/peripherals/gpio) and the relevant [connection bus](../../../../hardware/peripherals).

If you want to know more about what is happening in each step in the Toit program, see [here](../../../../hardware/drivers/bme280#i2cbus).
...
View the device log in the [Toit console](https://console.toit.io), in the **Logs** tab of the device or using the CLI command (showing logs from 15 minutes ago and trailing):
