...
The manufacturer data field is a custom payload that can be sent with a advertisement. The first two bytes is the company ID, followed by the payload. The sequence `[0xFF, 0xFF]` can be used for testing purposes.
