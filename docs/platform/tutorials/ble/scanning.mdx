...
being represented by [Bluetooth SIG](https://www.bluetooth.com/specifications/assigned-numbers/). As an example, the 16-bit UUID `0x190F` represents a battery service.
...
main:
  ...
  device.scan --duration=SCAN_DURATION: | remote_device/ble.RemoteDevice |
    if remote_device.service_classes.contains BATTERY_SERVICE:
