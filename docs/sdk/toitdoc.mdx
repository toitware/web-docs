...
2. `/** ... */` - Multiline comments with two stars at the beginning.
...
Multiline comments are the preferred documentation for longer comments. The individual lines should _not_ have any leading `*`, and should be aligned with the first `/`.
...
<!-- - Imported code blocks: ```` ``["file.toit":token]`` ````. In this case Toitdoc opens the referenced file, and extracts the code that is delimited by `/*token - start*/` and `/*token - end*/`. This is the preferred way of including longer examples. It also gives the possibility to move Toitdoc examples into test files. -->
...
<!-- * `$foo("foo.toit":id)` links to an element that is not in scope, but declared in the given file. Semantically, it's equivalent to searching for `id` in the scope introduced by importing "foo.toit".
    * `$foo(foo x y)` resolves to the element `foo` that takes 2 arguments, but only shows `foo` in the emitted Toitdoc.
    * `[Foo](class Foo)` links to the class name, instead of the constructor. If the class doesn't contain a default constructor (with no arguments), then this is the default. -->
...
In instance members and constructors, use "this instance" when referring to `this`. For example, "Calls the given $block on each element of this instance". This convention has the advantage that the comment is still valid for subclasses (compared to the alternative of referring to the classname).
