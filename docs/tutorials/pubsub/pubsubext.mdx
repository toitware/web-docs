...
[PubSub](../../../apis/pubsub) is a service for bi-directional communication. In this tutorial, we show how two apps can communicate with each other through the Toit cloud. Communicating through the cloud allows apps installed on different devices to communicate with each other. Note that in this scenario, the publish/subscribe events relies on the Max Offline value set for the device(s) involved.

## The subscriber

In the previous [tutorial](../pubsubint) we showed how the publish/subscribe service works in Toit and gave an example of how to run a subscriber and a publisher on your device.

This time, we create the subscriber as an application that will be installed on your device. 
...
The Toit program for subscribing to a cloud topic can be found here:
...
The corresponding app specification file for the subscriber that defines the pubsub trigger, and which is deployed on your device as a Toit application, can be found here:
...
Then run the publisher program from another device by pasting the following program in the Code tab of the device in the Toit console:
...
Go to the Toit console, and choose the device on which the subscriber
app was installed on. Look in the LOGS tab of the device to confirm that the message has been received.
