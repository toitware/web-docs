...
convenient way to manage multiple programs on a device. This makes
it possible to safely update individual programs without losing
confidence that the stability of the device is compromised.
...
(potentially downloading them from git) and bundle the resulting
