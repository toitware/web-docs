...
The following example shows how to scan for 3 seconds for the identifiers of remote devices that implement a battery service.
...
main:
  ...
  central.scan --duration=SCAN-DURATION: | device/ble.RemoteScannedDevice |
    if device.data.services.contains BATTERY-SERVICE:
