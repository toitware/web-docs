# Secrets
In this tutorial we will learn how to use secrets in Toit projects.
...
copy the `client-template.toit` file and replace `<API_KEY>` with the
actual API key.
...
Since we still have a `main` function that takes an `api_key` argument,
