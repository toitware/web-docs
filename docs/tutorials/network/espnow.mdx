...
The program starts by creating a service, followed by adding the broadcast
address as a peer. This is necessary since ESP-NOW can only send data to
peers that have been added to the peer list. This includes the broadcast
address, which is just a special address that represents all devices in range.
