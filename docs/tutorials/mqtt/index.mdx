...
identify the client. This is done using a `CLIENT_ID`. The client ID is
...
When a client connects to the broker, it can also specify a `CLEAN_SESSION`
...
With the `CLEAN_SESSION` flag, a client may also decide not to provide a
`CLIENT_ID`. In this case, the broker will generate a random ID for the
client.
...
<!-- HIDDEN CODE   MY_USERNAME ::= "" -->
<!-- HIDDEN CODE   MY_PASSWORD ::= "" -->
```
...
<!-- RESET CODE -->
<!-- HIDDEN CODE import net -->
<!-- HIDDEN CODE import mqtt -->
<!-- HIDDEN CODE import certificate_roots -->
<!-- HIDDEN CODE import encoding.json -->
<!-- HIDDEN CODE HOST ::= "" -->
<!-- HIDDEN CODE CLIENT_ID ::= "" -->
<!-- HIDDEN CODE TOPIC ::= "" -->
```
main:
  network := net.open
...
## TLS
For secure connections to the broker, we need to use TLS. The `TcpTransport` can
be used to connect to secure servers. For that we need root certificates which are
then used to authenticate the server.
...
2. Try to publish and subscribe to topics with different QoS levels.
    ...
    2. Send messages with QoS 1 but subscribe with `--max_qos=0` and see what happens.
