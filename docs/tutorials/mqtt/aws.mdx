...
For clarity we have moved the transport creation out of the `main` function.
However, all the parameters to the `mqtt.TcpTransport.tls` constructor can
also be passed directly to the `mqtt.Client` method:
...
This has the advantage that the transport is owned by the client and will
be closed automatically when the client is closed.
