...
If you don't have any resistors or the LED, you may skip them, but it might
damage your ESP32. The HC-SR04 is designed to run on 5V (and is known to be
flaky at lower voltages). ESP32 boards (usually) has a 5V output (often labeled
as VIN), but the ESP32 is not 5V tolerant on its inputs (depending on who you ask).
...
around 2V. The second 100kΩ resistor is just a pull-down resistor that ensures
that the TRIG pin is low when not in use, and doesn't trigger the sensor when
