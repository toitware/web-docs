...
- [Windows](https://archive.toit.io/cli/latest/windows/installer)

- [macOS](https://archive.toit.io/cli/latest/darwin)

- [Linux](https://archive.toit.io/cli/latest/linux)
...
With your ESP32 ready for use, the first step is to [initially provision and claim](../connect/provision) your ESP32.
...
<Note title="ESP32 not claimed?">

If the ESP32 was offline during the provisioning, for example if the WiFi signal was not in reach of your hardware, the automatic claiming into the Toit console will fail. 

You can always later claim the ESP32 with the following CLI command, when the WiFi is available and the ESP32 is online:

```shell
toit device claim <hardware-ID> --name MyToitDevice
```

You must manually provide the hardware ID in the `claim` command, which is printed in the output of the `provision` command, or if you run the command 
...
The provisioned ESP32, claimed into your organization in the Toit console, can be seen in the list of available devices with the CLI command:
