...
Note that the `package.lock` file should be shared with other developers of the program, and should therefore be checked into the Version Control System.
...
Note that only programs and local packages are allowed to depend on other local packages. Once a package is published it can only depend on Git packages.
