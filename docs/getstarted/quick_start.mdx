...
Create a free Toit account to get access to the Toit cloud. Go to [toit.io](https://toit.io/), click the `Start Now` button, and follow the sign-up instructions.

## 2. Install Toit

Next, install the Toit command-line interface (CLI), which is used to prepare your ESP32 with Toit and to deploy your Toit apps. Download [here](../download).
...
On **Windows**, the Toit CLI is automatically added to PATH by the installer.
...
Go to the Serial menu in the web-based [Toit console](https://console.toit.io/serial) to initially provision your hardware with the Toit firmware.
...
Open a terminal window, and replace `MY_SSID` and `my_password` with your own WiFi credentials and run the following Toit CLI command in order to 
provision your hardware with the Toit firmware:
...
on your ESP32 or dive into the [Toit language guide](../../language/language).
