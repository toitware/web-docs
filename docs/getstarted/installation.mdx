...
Depending on which shell you are using: edit either the `.zshrc` file or the `.bashrc` file.
...
Next, uncompress the downloaded Toit CLI, which is packed as a `tgz` file. Open a terminal window and navigate to the directory where you moved the downloaded file to.
...
Depending on which shell you are using: edit either the `.rczsh` file or the `.bashrc` file, located in your home directory.
