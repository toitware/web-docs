...
You can optionally specify the pod this should be based on
by passing a `--pod` argument like:

``` sh
artemis fleet group create solar-farm-dk-aar --pod esp32s3-cellular@v1.0.3
```

It is also possible to use an existing group as a template. In this case,
