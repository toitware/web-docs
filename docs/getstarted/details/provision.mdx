...
Provisioning means that the hardware is being granted a unique hardware ID used for the remaining of its lifecycle,
and the latest released Toit firmware will be installed on the hardware.
...
If you still have trouble connecting your ESP32 via a serial port to your computer, please visit the [troubleshoot](../troubleshoot) section.

</Note>

Follow the steps to initially provision your hardware:

- If you haven't installed the Toit CLI on your computer yet, follow the steps described [here](../../installation).
...
Provision your hardware using the [Web Serial interface](https://console.toit.io/serial) and follow the steps in the guide on the page.
...
- The provision process might take up to 5 minutes. When finished, it will output the following information:
...
claiming the hardware into your project
...
<Note title="Device models">

The `provision` command sets the model on the device. The default model is `esp32-4mb`.

If you want to use another device model, you can provide it with `-m` in the provision command and replace `model` with the specific device model to use:

```shell
toit serial provision -m model -p wifi.ssid="MY_SSID" -p wifi.password="my_password"
```

Look for available device models for each firmware version available in the
[Toit console](https://console.toit.io/project/firmware).
